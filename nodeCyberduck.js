...
CyberduckBot.prototype._init = function() {
	...
	var synPatterns={};
	if ((cyberduckSynons) && (typeof cyberduckSynons == 'object')) {
		for (var i in cyberduckSynons) synPatterns[i]='('+i+'|'+cyberduckSynons[i].join('|')+')';
	}
	// check for keywords or install empty structure to prevent any errors
	if ((!cyberduckKeywords) || (typeof cyberduckKeywords.length == 'undefined')) {
		cyberduckKeywords=[['###',0,[['###',[]]]]];
	}
	...
	if ((cyberduckPres) && (cyberduckPres.length)) {
		var a=new Array();
	...
	if ((cyberduckPosts) && (cyberduckPosts.length)) {
		var a=new Array();
	...
	if ((!cyberduckQuits) || (typeof cyberduckQuits.length == 'undefined')) {
		cyberduckQuits=[];
	}
	// done
...
CyberduckBot.prototype._postTransform = function(s) {
	...
	s=s.replace(/\s{2,}/g, ' ');
	s=s.replace(/\s+\./g, '.');
	if ((this.cyberduckPostTransforms) && (cyberduckPostTransforms.length)) {
